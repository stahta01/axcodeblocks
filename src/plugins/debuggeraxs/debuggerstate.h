...
{
    public:
        DebuggerState(DebuggerAXS* plugin);
        ~DebuggerState();
        ...
        cb::shared_ptr<DebuggerBreakpoint> AddBreakpoint(unsigned long addr, bool temp = false,
                                                  const wxString& lineText = wxEmptyString);
        ...
        int HasBreakpoint(unsigned long addr, bool temp); // returns -1 if not found
